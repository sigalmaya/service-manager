	...
	AdditionalTests: func(ctx *common.TestContext, t *test.TestCase) {
		Context("additional non-generic tests", func() {
			var (
				...
				postBrokerRequestWithInvalidBase64TlsEncoding common.Object
				postBrokerRequestWithdTls                     common.Object
			)
			...
			Describe("POST", func() {
				...
				Context("when tls credentials are valid", func() {
					It("returns 400", func() {
						reply := ctx.SMWithOAuth.POST(web.ServiceBrokersURL).WithJSON(postBrokerRequestWithdTls).
							Expect().
